...
export async function POST(request: Request) {
  try {
    ...
    // If no spreadsheetId provided, we search for one named 'Murrabi Expenses Master' inside the Root Folder
    if (!targetId) {
       ...
       const searchRes = await drive.files.list({
         q: `name = 'Murrabi Expenses Master' and '${rootId}' in parents and mimeType = 'application/vnd.google-apps.spreadsheet' and trashed = false`,
         fields: 'files(id)',
       });
       ...
       if (searchRes.data.files && searchRes.data.files.length > 0) {
         ...
         // Move to Root Folder (since create doesn't support parents in sheets v4 directly)
         await drive.files.update({
           fileId: targetId!,
           addParents: rootId,
           removeParents: 'root', // removes from the root of Drive
           fields: 'id, parents',
         });
         ...
         await sheets.spreadsheets.values.update({
           ...
           requestBody: {
             values: [['Date', 'Member Name', 'Month', 'Total Amount', 'Purpose', 'Status', 'Drive Folder', 'Email Sent']]
           }
         });
       }
    }
