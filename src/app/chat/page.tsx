    ...
    <div className="flex h-full w-full overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <div className="flex items-center justify-between px-4 lg:px-8 pt-4 lg:pt-8 pb-3 lg:pb-4 shrink-0 relative z-10">
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <div>
              <h1 className="text-xl lg:text-2xl font-black italic tracking-tighter text-black uppercase">MurrabiAI</h1>
          ...
          <div className="flex items-center gap-2 lg:gap-3">
            {messages.length > 0 && (
              <button onClick={clearChat} className="p-2 rounded-xl text-black/20 hover:text-red-400 hover:bg-red-600/10 transition-all" title="Clear conversation">
                <Trash2 size={16} />
              </button>
            )}
            ...
            <div className="flex items-center justify-between px-4 pb-3">
              ...
              <div className="flex items-center gap-2">
                {isLoading ? (
                  ...
                  <button onClick={() => sendMessage()} disabled={!input.trim()} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-red-600 disabled:bg-white/5 disabled:text-black/20 text-black text-xs font-black transition-all hover:bg-red-700 active:scale-95 shadow-lg shadow-red-900/30">
                    <Send size={12} /> Send
                  </button>
                )}
              </div>
      ...
      <div className={clsx(
        ...
        <div className="p-4 border-t border-white/5 shrink-0">
            ...
            {CONTEXT_OPTIONS.map(opt => (
              <button key={opt.id} onClick={() => toggleContext(opt.id)}
                className={clsx("w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border transition-all text-left",
                  activeContexts.has(opt.id) ? "bg-red-600/10 border-red-500/30 text-red-400" : "border-white/5 bg-white/[0.02] text-black/30 hover:text-black/60 hover:border-white/10"
                )}>
                <opt.icon size={13} />
          ...
          <p className="text-[9px] text-black/15 mt-3 leading-relaxed">Selected data will be sent with your next message.</p>
        </div>
      </div>
    </div>
  );
