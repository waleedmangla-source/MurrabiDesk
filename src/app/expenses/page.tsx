...
export default function ExpensesPage() {
  ...
  return (
    <div className="flex h-screen w-full bg-transparent overflow-hidden">
      {/* Past Expenses Sidebar (Inline) */}
      <div 
        className={clsx(
          "h-full border-r border-white/5 bg-[#020310]/40 backdrop-blur-3xl transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col shrink-0 relative z-20",
          isHistoryOpen ? "w-[400px] opacity-100 border-r" : "w-0 opacity-0 border-r-0"
        )}
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar no-drag pb-32 min-w-[400px]">
          {months.map(m => {
            ...
            return (
              <div key={m} className="space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <h3 className="text-[10px] font-medium tracking-[0.2em] text-white uppercase">{m}</h3>
                  <span className="text-[10px] font-medium text-red-500 uppercase tracking-widest">${monthTotal.toFixed(2)}</span>
                </div>
                  ...
                  {monthForms.map(form => (
                    <div key={form.id} className="glass bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col gap-3 relative group hover:border-red-600/30 hover:bg-white/10 transition-all">
                       <div className="flex items-start justify-between">
                          <div className="pr-4">
                            <p className="text-[10px] font-medium text-white truncate max-w-[180px]">{form.date} &bull; <span className="text-white/40">{form.fullName}</span></p>
                            <p className="text-[10px] text-white/50 mt-1.5 leading-relaxed">{form.purpose}</p>
                          </div>
                          <div className="flex flex-col items-end shrink-0">
                            <p className="text-xs font-medium text-white bg-black/40 px-2 py-1 rounded-lg border border-white/10 shadow-inner">${form.total.toFixed(2)}</p>
                          </div>
                       </div>
                       ...
                       <div className="mt-1 pt-3 border-t border-white/5 flex items-center justify-between">
                          <label className="flex items-center gap-3 cursor-pointer group/chk select-none">
                            ...
                            />
                            <span className={clsx(
                              "text-[9px] font-medium uppercase tracking-widest transition-colors",
          ...
          {expensesHistory.length === 0 && (
            <div className="text-center py-20 flex flex-col items-center">
               ...
               <p className="text-[12px] font-medium uppercase tracking-widest text-white/80">No History Found</p>
               <p className="text-[9px] font-medium uppercase tracking-widest text-white/30 mt-2 max-w-[200px] leading-relaxed">
      ...
      {/* Main Expense Tool Area */}
      <div className="flex-1 main-content flex flex-col gap-8 pb-12 animate-in fade-in duration-700 h-screen overflow-y-auto custom-scrollbar relative z-10">
